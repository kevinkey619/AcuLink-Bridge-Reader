...
namespace AcuLink_Bridge_Reader_CSharp
{
    public partial class frmMain : Form
    {
            ...
            if (!BackgroundWorker1.CancellationPending == true)
            {
                try
                {
                    ...
                    if (packet.Ethernet.IpV4.Tcp.Payload != null)
                    {
                        ...
                        if ((Properties.Settings.Default.filterOnSensorId.ToString().Length > 0 && Properties.Settings.Default.filterOnSensorId.ToString() == sensor))
                        {
                            match = Regex.Match(text.ToLower(), "sensor=" + Properties.Settings.Default.filterOnSensorId.ToString() +
                                "&mt=" + Properties.Settings.Default.sensorType.ToString() + "|mt=pressure", RegexOptions.Singleline);
                        }
                        else
                        {
                        ...
                        if (match.Success)
                        {
                            ...
                            if (pressure > 0 & humidity > 0 & noSensorDataIterations < 5)
                            {
                                if (Properties.Settings.Default.postToWunderground == true)
                                {
                                    string wundergroundUpdateString = "http://rtupdate.wunderground.com/weatherstation/updateweatherstation.php?ID=" + Properties.Settings.Default.wuStation +
                                        ...
                                        "&softwaretype=" + "Kevins%20Acu-Rapid%201217&action=updateraw&realtime=1&rtfreq=15" + "&windgustmph=" + windGust;
                                    try
                                    {
                                ...
                                {
                                    string newFileName = "weather.csv";
                            ...
                            this.Invoke(new MethodInvoker(() => txtOutput.Text = DateTime.Now + "   " + "T:" + temperature.ToString("F1") + "   " + "H:" + humidity + "   " + "W:" +
                                ...
                                "  " + "RHR:" + rainHour.ToString("F2") + "   " + "BAR:" + pressure.ToString("F2") + "   " + "DEW:" + dewpoint.ToString("F1") + "   " + "S:" + sensor +
                                "   " + "WU: " + wuResponse + "\t" + "WB: " + wBugResponse + "\t" + "PWS: " + pwsResponse + System.Environment.NewLine + txtOutput.Text));
                            ...
                            {
                                this.Invoke(new MethodInvoker(() => txtOutput.Text = DateTime.Now + "\t" + "INFO: Not receiving data from 5-in-1 sensor. Low batteries? Too far away?" +
                                    System.Environment.NewLine + txtOutput.Text));
                                battery = "unknown";
                                signal = 0;
                                signalFails += 1;
        ...
        {
            MessageBox.Show("Kevin's Acu-Link Bridge to Weather Underground and Weatherbug Rapid Fire Updater" + System.Environment.NewLine + "Version: " +
                Assembly.GetExecutingAssembly().GetName().Version.ToString() + System.Environment.NewLine + "© 2013  Kevin Key" +
